...
void __global__ addMaxtrix2Dto2D(int *a,  int *b,  int *c);
...
int main(int argc, char *argv[]){
    ...
    printf("cpu addMatrix 2D&2D(%d & %d) time: %.10f seconds\n",nX, nY, iElaps);
    ...
    iStart = cpuSecond();
    addMaxtrix2Dto2D<<<grid_size, block_size>>>(d_a, d_b, d_c);
    cudaDeviceSynchronize();
    iElaps = (cpuSecond() - iStart);
    printf("gpu addMatrix 2D&2D(%d & %d) time: %.10f seconds\n",nX, nY, iElaps);
